...
class _CabangScreenState extends State<CabangScreen> {
  ...
  void _filter() {
    ...
    setState(() {
      _filtered = q.isEmpty
          ? List.of(_all)
          : _all.where((c) {
              return (c['nama'] as String? ?? '').toLowerCase().contains(q) ||
                  (c['alamat'] as String? ?? '').toLowerCase().contains(q);
            }).toList();
    });
  }
  ...
  void _openSheet({Map<String, dynamic>? item}) {
    showModalBottomSheet(
      ...
      builder: (_) => _CabangSheet(item: item, onSaved: _load),
    );
  }

  Future<void> _toggleActive(Map<String, dynamic> item) async {
    try {
      await SupabaseService().updateCabang(
        id: item['id'].toString(),
  ...
  Future<void> _delete(String id) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('Yakin ingin menghapus cabang ini?'),
        actions: [
          TextButton(
    ...
    if (ok != true) return;
    try {
      await SupabaseService().deleteCabang(id);
      _load();
    } catch (e) {
  ...
  Widget _emptyState() => Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              'Tap + untuk menambah cabang baru',
...
class _CabangSheet extends StatefulWidget {
  final Map<String, dynamic>? item;
  final VoidCallback onSaved;
  const _CabangSheet({this.item, required this.onSaved});

  @override
  State<_CabangSheet> createState() => _CabangSheetState();
}

class _CabangSheetState extends State<_CabangSheet> {
  ...
  Future<void> _save() async {
    ...
    try {
      if (widget.item == null) {
        await SupabaseService().addCabang(
  ...
  Widget _saveButton(bool isEdit) => SizedBox(
        ...
        child: ElevatedButton(
          ...
          child: _saving
              ...
              : Text(isEdit ? 'Simpan Perubahan' : 'Tambah Cabang'),
        ),
      );
